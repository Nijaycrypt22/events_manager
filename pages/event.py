...
def show_create_event_page():
    ...
    show_navbar()
    with ui.element("div").classes("w-full h-full items-center flex flex-col bg-white"):
        ...
        with ui.card().classes("font-bold shadow-xl w-3/5"):
            ...
            event_title =  ui.input(placeholder="Enter your title").props("outlined dense").classes("w-full")
            ui.label("Event Venue").classes("text-slate-500 font-normal text-sm")
            event_venue = ui.input(placeholder="Enter your venue").props("outlined dense").classes("w-full")
           
            # Start and End Time section with time inputs
            with ui.row().classes("w-full justify-between gap-4"):
                with ui.element('div').classes("flex flex-col grow"):
                    eVent_start_time = ui.label("Start time").classes("text-slate-500 font-normal text-sm")
                    ui.input().props("outlined dense type=time").classes("w-full")
                with ui.element('div').classes("flex flex-col grow"):
                    event_end_time = ui.label("End time").classes("text-slate-500 font-normal text-sm")
                    ui.input().props("outlined dense type=time").classes("w-full")
            ...
            with ui.row().classes("w-full justify-between gap-4"):
                with ui.element('div').classes("flex flex-col grow"):
                    ...
                    ui.input().props("outlined dense type=date").classes("w-full")
                with ui.element('div').classes("flex flex-col grow"):
    ...
    with ui.element("div").classes("w-full items-center flex flex-col bg-slate-100 mt-10"): # Added a positive margin for separation
        ...
        with ui.card().classes("font-bold shadow-xl mb-8 w-3/5"):
            ...
            ui.upload(label="Click or drop an image to upload").classes("w-full h-48 rounded")
            ui.label("Event Description").classes("text-slate-500 font-normal text-sm mt-4")
            ui.input(placeholder="Type here...").props("outlined dense type=textarea").classes("w-full h-32")
